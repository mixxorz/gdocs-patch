...
class SectionColumnParser(GDocParser[SectionColumn]):
    def parse(self, data: Any) -> SectionColumn:
        return SectionColumn(
            ...
            padding_end=dimension_parser.parse(data["paddingEnd"]),
        )
...
class SectionStyleParser(GDocParser[SectionStyle]):
    def parse(self, data: Any) -> SectionStyle:
        return SectionStyle(
            columns=(
                [
                    section_column_parser.parse(column)
...
class SectionBreakParser(GDocParser[SectionBreak]):
    def parse(self, data: Any) -> SectionBreak:
        return SectionBreak(style=section_style_parser.parse(data["sectionStyle"]))


section_column_parser = SectionColumnParser()
section_style_parser = SectionStyleParser()
section_break_parser = SectionBreakParser()
